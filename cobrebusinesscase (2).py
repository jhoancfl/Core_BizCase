...
We are gonna star with a review of the feautures in order to have a better understanding about their caracteristics and/or limitations in order to get the insights for the next analysis.
We are gonna star with the actors (nodes), our first exploring is about the target. We have three labels according to the metadata: nodes has been labeled as being created by a "licit", "illicit" or "unknown" entity.
"""
...
plt.title('Class Frequencies in elliptic_txs_classes')
plt.ylabel('Frequency')
plt.xlabel('Class')

# Annotate the bars with percentages
for i, v in enumerate(class_summary['Frequency']):
    plt.text(i, v + 100, f"{class_summary['Percentage'][i]:.2f}%", ha='center', va='bottom')
...
for i, (index, row) in enumerate(top_5_components.iterrows()):
    ...
    pos = nx.spring_layout(subgraph, seed=42)  # Use a consistent seed for layout
...
# Function to calculate statistics for a given DataFrame and class
...
# Now you can use these top_variables in your plotting code
import matplotlib.pyplot as plt
...
# Sampling again, a smaller dataset needed in order to run DOMINANT
sampled_nodes = elliptic_txs_classes.sample(frac=0.001, random_state=420)
sampled_tx_ids = sampled_nodes['txId'].tolist()
sampled_features = elliptic_txs_features[elliptic_txs_features['txId'].isin(sampled_tx_ids)]
sampled_edgelist = elliptic_txs_edgelist[
    elliptic_txs_edgelist['txId1'].isin(sampled_tx_ids) | elliptic_txs_edgelist['txId2'].isin(sampled_tx_ids)
...
# You might need to add node features to the PyG data object.
# If your sampled_features DataFrame has features associated with each node:
# 1. Create a node feature matrix from the DataFrame
# Get node features and create a tensor from the sampled_features
...
# prompt: delete the nodes with Unknown condition in all the data bases edges, classes and features
...
# Assuming 'test_pred' and 'data.y[data.test_mask]' are your predictions and true labels respectively
...
# Assuming your metrics are stored in variables like accuracy, roc_auc, f1, recall, gini
metrics = {
...
for i, feature in enumerate(top_5_features):
    plt.subplot(2, 3, i + 1)
    plt.hist(class1_data[feature], bins=30, alpha=0.5, label='Class 1', density=True)
    plt.hist(class2_data[feature], bins=30, alpha=0.5, label='Class 2', density=True)
    plt.xlabel(feature)
    plt.ylabel('Density')
